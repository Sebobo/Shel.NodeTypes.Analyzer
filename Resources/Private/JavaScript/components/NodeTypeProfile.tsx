    ...
    return (
        <div className={classes.nodeTypeProfile}>
            ...
            <section className={classes.nodeTypeConfigurationCard}>
                <header onClick={() => setShowProperties(!showProperties)}>Properties</header>
                {properties ? showProperties ?
                    ...
                    </ul>  : '' : <span>No properties defined</span>
                }
            </section>
            <section className={classes.nodeTypeConfigurationCard}>
                <header onClick={() => setShowSupertypes(!showSupertypes)}>Supertypes</header>
                {superTypes ? showSupertypes ?
                    <ul>
                        {Object.keys(superTypes).map(superTypeName =>
                            <li key={superTypeName}>{superTypeName}</li>
                        )}
                    </ul>  : '' : <span>No supertypes defined</span>
                }
            </section>
            <section className={classes.nodeTypeConfigurationCard}>
                <header onClick={() => setShowAllowedChildNodes(!showAllowedChildNodes)}>Allowed child node types
                </header>
                {allowedChildNodeTypes.length ? showAllowedChildNodes ?
                    <ul>
                        {allowedChildNodeTypes.map((nodeTypeName, index) =>
                            <li key={index}>{nodeTypeName}</li>
                        )}
                    </ul>  : '' : <span>No childnodes allowed</span>
                }
            </section>
            <section className={classes.nodeTypeConfigurationCard}>
                <header onClick={() => setShowAllowedGrandChildNodes(!showAllowedGrandChildNodes)}>Allowed grandchild
                    node types
                </header>
                {childNodes ? showAllowedGrandChildNodes ?
                    <ul>
